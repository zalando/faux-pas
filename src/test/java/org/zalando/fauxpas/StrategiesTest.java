...
public final class StrategiesTest {
    ...
    public void shouldProvideDefaultLoggingStrategy() {
        final LoggingStrategy unit = (LoggingStrategy) Strategies.logging();
        assertThat(unit, is(notNullValue()));
    }

    @Test
    public void shouldProvideDefaultRethrowStrategy() {
        final Strategy unit = Strategies.rethrow();
    ...
    public <X extends Throwable> void shouldNotWrap(final X expected) {
        final Function<Throwable, RuntimeException> unit = Strategies.unchecked();
